...
struct rt_spi_device *rf_pipe_radio_spi_init(void)
{
    rt_err_t res;
    static struct rt_spi_device *radio_spi_device;
    rt_hw_spi_device_attach("spi2", "rf_pipe", GPIOA, GPIO_PIN_9);
    radio_spi_device = (struct rt_spi_device *)rt_device_find("rf_pipe");
    if (!radio_spi_device)
    {
        LOG_D("spi_device_attach failed! cant't find rf_pipe device!\n");
        return RT_NULL;
    }
